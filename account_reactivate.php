...
{
 	$btfb_response              = $btfb_obj_authentication -> sendAccountReactivationRequestForExcessLimit();
 	$btfb_array_value['center'] = $btfb_response; 		
}
else 
{
  if($_SESSION['System_Language']=="serbian")
  $btfb_array_value['center'] = $btfb_obj_template->showRegForm(ROOT_LANG_TEMPLATE_PATH."serbian/account_reactivation_for_exceed_limit.html",$btfb_array_value);		
  else 
  $btfb_array_value['center'] = $btfb_obj_template->showRegForm(ROOT_LANG_TEMPLATE_PATH."english/account_reactivation_for_exceed_limit.html",$btfb_array_value);		
}
